...
export async function getStaticProps() {
  const tokenAddress = fs.readFileSync("./src/utils/RTK_address.txt", "utf-8");
  return {
    props: {
      tokenAddress,
    },
...
function Faucet({ tokenAddress }) {
  ...
  const requestApproval = async () => {
    ...
    const response = await tokenContract.approve(
      address,
      ethers.utils.parseEther(RTKAmount.toString()),
      { gasLimit: 100000 }
    );

    // Update the UI
    setTokens(RTKAmount.toString());
  ...
  const requestTokens = async () => {
    ...
    const approval = await tokenContract.connect(wallet).approve(
      address,
      ethers.utils.parseEther(RTKAmount.toString()), // 0.1 RTK
      { gasLimit: 100000 }
    );

    console.log("approval", approval);

    // Transfer 0x90F79bf6EB2c4f870365E785982E1f101E93b906 RTK to the user's address
  ...
  return (
    <div className="App">
      ...
      <br />
      <label>
        Tokens: <input type="text" value={RTKAmount} readOnly />
      </label>
      <br />
